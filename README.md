...
| Tests | pytest, 229 tests, real Postgres | Separate `_test` database, LLM faked via dependency override |
...
├── services/          budget, target, categorization, receipt-parsing and currency maths (pure, no I/O); ledger (per-account and total balance queries)
