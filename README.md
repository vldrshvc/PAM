...
| Tests | pytest, 212 tests, real Postgres | Separate `_test` database, LLM faked via dependency override |
...
| POST | `/receipts/scan?tz=` | A receipt photo in, `{total, merchant, date, category, fell_back, converted}` out — `total` always euro, `converted` carrying the original amount, currency and ECB rate when it was not; never writes, never stores the photo |
...
- **Foreign receipts are converted at the ECB's published rate, not a live quote.** The euro reference rates are free, need no key, and are what a set of Irish books would use. The ninety-day file rather than today's, so a receipt is converted at the rate in force the day it was printed; there is no rate on a weekend, so a Saturday receipt takes Friday's, which is what an accountant does. The table is fetched once and held for six hours, and when the ECB cannot be reached the last good copy keeps being served — a day-old reference rate beats refusing to read a receipt. Rates are never stored: the converted euro amount is, because the ledger has exactly one currency. `app/ecb.py` fetches, `app/services/fx.py` does the maths.
